...
function SiteHeader() {
  return (
    <header style={{ padding: '16px 40px', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', position: 'sticky', top: 0, backgroundColor: '#0f172a', zIndex: 50, backdropFilter: 'blur(10px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 64 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src='/assets/icons/splash-icon-light.png' alt='Pinglo Logo' style={{ width: 28, height: 28 }} />
          <span style={{ fontSize: 20, fontWeight: 800, color: '#a78bfa' }}>Pinglo</span>
        </div>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} style={{ fontSize: 16, fontWeight: 500, color: '#cbd5e1', textDecoration: 'none', transition: 'color 0.2s' }} onMouseEnter={(e) => (e.currentTarget.style.color = '#a78bfa')} onMouseLeave={(e) => (e.currentTarget.style.color = '#cbd5e1')}>
      {children}
    </a>
  )
}
...
function SiteFooter() {
  return (
    <footer style={{ padding: '48px 40px', borderTop: '1px solid rgba(255, 255, 255, 0.1)', backgroundColor: '#0a0f1e' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '40px', marginBottom: 40 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: '#94a3b8', lineHeight: 1.6 }}>Keep the people who matter close. A simple way to stay in touchwithout the clutter.</p>
          </div>
          <div>
        ...
        <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.1)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <p style={{ fontSize: 14, color: '#94a3b8', margin: 0, marginBottom: 8 }}> 2025 Pinglo. All rights reserved.</p>
            <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>Available on iOS and Android  download and start reconnecting today.</p>
          </div>
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} style={{ fontSize: 14, color: '#94a3b8', textDecoration: 'none', transition: 'color 0.2s' }} onMouseEnter={(e) => (e.currentTarget.style.color = '#a78bfa')} onMouseLeave={(e) => (e.currentTarget.style.color = '#94a3b8')}>
      {children}
    </a>
  )
}
